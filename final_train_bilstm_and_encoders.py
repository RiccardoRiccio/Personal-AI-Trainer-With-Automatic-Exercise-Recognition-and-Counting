...
joblib.dump(scaler, 'thesis_bidirectionallstm_scaler.pkl')
joblib.dump(label_encoder, 'thesis_bidirectionallstm_label_encoder.pkl')
...
def create_lstm_model(units=100, dropout_rate=0.5, learning_rate=0.001, regularizer=l2(0.01)):
    model = Sequential()
    model.add(Bidirectional(LSTM(units, return_sequences=True, kernel_regularizer=regularizer), input_shape=(n_timesteps, n_features)))
    model.add(Dropout(dropout_rate))
    model.add(Bidirectional(LSTM(units, kernel_regularizer=regularizer)))
    model.add(Dropout(dropout_rate))
...
# Save the best model
best_model.save('final_forthesis_bidirectionallstm_and_encoders_exercise_classifier_model.h5')
